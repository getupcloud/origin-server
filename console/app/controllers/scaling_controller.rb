class ScalingController < ConsoleController
  ...
  def update
    ...
    if @cartridge.save
      redirect_to application_scaling_path, :flash => flash_messages(@cartridge.messages).merge({:success => _("Updated scale settings for cartridge '%s'") % @cartridge.display_name})
    else
      render :show
    end
  end
