class StorageController < ConsoleController
  ...
  def update
    ...
    if @cartridge.save
      redirect_to application_storage_path(@application), :flash => flash_messages(@cartridge.messages).merge({:success => _("Updated storage for cartridge '%s'") % @cartridge.display_name})
    else
